 ...
 const Capture = () => {
   ...
   const handleAnalyze = async () => {
     ...
     try {
       ...
       toast.success("Room analyzed! Let's start your challenges!");
         ...
         // Start generating vision in background
   ...
   const generateVision = async (image: string, selectedIntent: string, currentRoomId: string) => {
     ...
     try {
       ...
       if (response.error) {
         console.error("Vision generation error:", response.error);
       ...
       if (generatedImage) {
         setVisionImage(generatedImage);
         
         // Save to database
         await supabase
           .from("rooms")
           .update({ after_image_url: generatedImage })
           .eq("id", currentRoomId);
